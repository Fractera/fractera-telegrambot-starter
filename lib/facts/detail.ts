...
export type FactDetail = {
  ...
  lost: string
  /**
   * 🤖 От чего зависит истинность факта — шестая строка (141-1).
   *
   * 🔒 ПОРОЖДАЕТСЯ, А НЕ ПИШЕТСЯ РУКАМИ: берётся из `scope` записи и заголовков
   * признаков, на которые он ссылается. Рукописное название города разошлось бы
   * с реестром в день переименования.
   *
   * 🔒 ПУСТОЙ СПИСОК ЗНАЧИТ «ЗАВИСИМОСТЬ НЕ ОПИСАНА», А НЕ «ВЕРЕН ВЕЗДЕ», и
   * экран обязан сказать это словами — иначе беспризорный факт молча повышается
   * до универсального.
   */
  scope: string[]
}
...
/**
 * От чего зависит признак — ЗАГОЛОВКАМИ, а не голыми ключами.
 *
 * 🔒 КЛЮЧ, НА КОТОРЫЙ НЕКОМУ СОСЛАТЬСЯ, ПОКАЗЫВАЕТСЯ КАК ЕСТЬ И С ПОМЕТКОЙ.
 * Молча выбросить его нельзя: человек увидел бы четыре зависимости вместо пяти
 * и не узнал, что одна сломана. Сборку на этом останавливает `check:registry` —
 * здесь же лишь честный показ того, что уже случилось.
 */
function scopeOf(fact: Fact, all: Fact[]): string[] {
  return (fact.scope ?? []).map(key => {
    const found = all.find(f => f.key === key)
    return found ? `${found.title} (${key})` : `${key} — записи с таким ключом нет`
  })
}
 ...
 */
export function factDetail(fact: Fact, all: Fact[] = []): FactDetail {
  return {
