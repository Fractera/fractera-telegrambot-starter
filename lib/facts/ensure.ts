...
/**
 * Провести таблицу по лестнице колонок ОДИН РАЗ ЗА ЖИЗНЬ ПРОЦЕССА (162-2).
 *
 * ✗ ЧЕМ ОПЛАЧЕНО, И ЭТО ЖИВОЙ ДЕФЕКТ, А НЕ ПРЕДОСТОРОЖНОСТЬ. Измерено
 * 2026-09-08: лестницу звал ТОЛЬКО писатель (`write.ts`), и таблицы, в которые
 * после 161-2 никто не писал, остались старой формы — шесть из девяти:
 * `fact_person_name`, `timezone`, `tone`, `address_form`, `important_people`,
 * `nationality`. Чтение перечисляет колонки поимённо, поэтому
 * `SELECT … claim, basis …` падал ЦЕЛИКОМ: `no such column: claim`. Снаружи это
 * выглядело как «слой данных не ответил», то есть как поломка службы, а на деле
 * шесть признаков личной памяти были нечитаемы вовсе.
 *
 * 🔒 ЗАКОН ПРОЕКТА ЗВУЧИТ ТАК: КОЛОНКА ДОБАВЛЯЕТСЯ ЧЕТЫРЬМЯ ПРАВКАМИ — форма,
 * лестница, образец И ВЫЗОВ ЛЕСТНИЦЫ У КАЖДОГО, КТО ЧИТАЕТ ПОИМЁННО. Четвёртая
 * отсутствовала. Тот же класс оплачен в 143.
 *
 * 🔒 ПАМЯТЬ НА ПРОЦЕСС, А НЕ НА ВЫЗОВ: первое чтение таблицы платит два `ALTER`
 * (десятки мс), следующие — ноль. Цель владельца — реактивный первый уровень, и
 * лестница на каждом чтении сделала бы дешёвый уровень дорогим.
 * 🛑 ЦЕНА ЭТОГО ВЫБОРА НАЗВАНА: таблица, изменённая ИЗВНЕ после старта процесса,
 * второй раз не проверяется до перезапуска службы.
 */
const laddered = new Set<string>()

export async function ensureLateColumns(table: string): Promise<string[]> {
  if (laddered.has(table)) return []
  laddered.add(table)
  return addLateColumns(table)
}
