...
export async function read(input: {
  ...
  if (depth >= 2 && query) {
    ...
    // 🔒 ЯКОРЕЙ НЕТ — СПРАШИВАЕМ ИСХОДНОЙ ФРАЗОЙ И ГОВОРИМ ОБ ЭТОМ. Молчаливый
    // откат к прежнему пути неотличим от работающего обогащения.
    const asking = anchors.length > 0 ? anchors : [query]
    ...
    levels.push({
      ...
      note:
        anchors.length > 0
          ? "спрошено именами, найденными на уровне 1"
