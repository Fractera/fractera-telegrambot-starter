...
export async function read(input: {
  ...
  if (deep && query) {
    // 🔒 РЕЖИМ `local`, А НЕ УМОЛЧАНИЕ: измерено 161-1, что свежая запись находится
    // им и НЕ находится `hybrid`, который подмешивает медленно пересобираемые
    // обобщения по всему корпусу.
    const answer = await ask(query, "local")
