...
// восстанавливается апекс (`admin.example.com` → `example.com`) в доменном режиме.
// 🔒 `chat` ДОБАВЛЕН В СПИСОК ШАГОМ 137. Без него `apexFrom("chat.example.com")` вернул бы
// сам себя, и построенный из него адрес стал бы `chat.chat.example.com` — отказ, видимый
// только тому, кто откроет ссылку со страницы бота.
export const KNOWN_PREFIXES = ["www", "auth", "admin", "data", "chat", "lightrag", "projects", "design"];
