...
// ЕДИНЫЙ ВХОД В ОБА РЕЕСТРА — ПЯТЬ ПРИМИТИВОВ (157-5, дополнен 158-5а).
//
// 🔒 ЧЕТЫРЕ ЧИТАЮТ, ПЯТЫЙ ПИШЕТ. Число исправлено ВМЕСТЕ с перечислением
// (158-8): шапка, говорившая «четыре», пережила появление `remember` на сутки —
// ровно тот класс, которым проект платил уже пять раз.
//
// 🔒 ЧИТАЮЩИХ ИМЕННО ЧЕТЫРЕ, И КОРПУС У НИХ ПАРАМЕТР. Отдельная дверь «есть ли
// инструмент под задачу» — это `find` по другому корпусу; заведи её отдельно, и
// через месяц у двух дверей разошлась бы форма ответа. Тот же вопрос по другому
// корпусу новым примитивом НЕ становится — а другой ГЛАГОЛ становится, и это
// единственная причина, по которой `remember` законен.
//
...
export type Hit = Pointer & {
  ...
  why: string[]
  /**
   * Совпало ли по СЛОВАМ ЧЕЛОВЕКА (триггеры и вопросы), а не только по имени и
   * описанию записи (165-3).
   *
   * 🔒 РАЗНИЦА УЖЕ СЧИТАЛАСЬ ЗДЕСЬ, ПРОСТО НЕ ВЫХОДИЛА НАРУЖУ. Порог `find()`
   * пропускает и слабое совпадение — два слова из описания, — и это правильно
   * для поиска. Но зовущему нужно знать, отвечает он на заданный вопрос или
   * принёс похожее: без этого различения «нашла не то» неотличимо от «нашла».
   * ✗ измерено 2026-09-09: на «в каком я часовом поясе» ответ был `found: true`
   * со значением ГОРОДА, а часовой пояс стоял в `missing`.
   */
  strong: boolean
}
...
export function find(
  ...
  for (const e of pointers(corpus)) {
    ...
    scored.push({
      hit: { ...bare(p), strong: hitStrong.size > 0, why },
      score: hitStrong.size * 2 + hitWeak.size,
    })
  }
  if (scored.length === 0) {
...
export async function recall(
  ...
  if (column) {
    ...
    // ── ЧТО ЗНАЧИТ «ЗНАЧЕНИЕ» У ЧУЖОЙ КОЛОНКИ (162-2) ─────────────────────
    //
    // ✗ НАЙДЕНО ИЗМЕРЕНИЕМ 2026-09-08 И БЫЛО УВЕРЕННОЙ ЛОЖЬЮ. Колонку
    // `tgdesk_entries.kind` делят ШЕСТЬ признаков — memo, note, task, receipt,
    // place, idea. Чтение «последнее непустое значение колонки» отвечало на
    // вопрос про ЧЕКИ значением `memo`, потому что последняя запись склада была
    // заметкой. Форма ответа при этом безупречная: ключ спрошен про чек, значение
    // выдано, никакой ошибки. Так же врали бы `raw_kind` (девять признаков) и
    // `tgdesk_artifacts.kind` (три).
    //
    // 🔒 ПРАВИЛО ВЫВОДИТСЯ ИЗ РЕЕСТРА, А НЕ ПИШЕТСЯ СПИСКОМ: колонку делят
    // несколько признаков → это ПЕРЕЧИСЛЕНИЕ, и признак означает одно конкретное
    // значение в ней (последний сегмент ключа: `entity.receipt` → `receipt`).
    // Признак в колонке один → значение и есть содержимое колонки.
    // Рукописный список «какие колонки перечисления» разошёлся бы с реестром
    // молча — тот же класс, что числа в инструкциях.
    const shared = allFacts().filter(f => {
      const p = placementOf(f)
      return p.kind === "column" && p.table === table && p.column === column
    })
    if (shared.length > 1) {
      const marker = wanted.split(".").pop() ?? wanted
      where.push(`${column} = ?`)
      params.push(marker)
    } else if (String(fact.valueType ?? "") === "flag") {
      // 🔒 ФЛАГ ЗНАЧИТ НАЛИЧИЕ, И НОЛЬ — ЭТО ОТСУТСТВИЕ, А НЕ ЗНАЧЕНИЕ. Иначе
      // ответом на «есть ли про деньги» стал бы честно записанный `0`, который
      // человек прочитает как «есть, ноль».
      where.push(`${column} IS NOT NULL AND ${column} <> 0 AND ${column} <> ''`)
    } else {
