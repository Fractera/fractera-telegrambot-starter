...
// ✗ `FRACTERA_SLOT_ENV` ДОБАВЛЕН ЗДЕСЬ (137-6, 2026-09-06), И БЕЗ НЕГО КЛЮЧ
// ANTHROPIC «НЕ БЫЛ ЗАДАН» ПРИ ЖИВОМ КЛЮЧЕ. `process.cwd()` в службе бота —
// `/opt/fractera/telegrambot`, то есть файл СЛУЖБЫ, а на 3000 тот же код читал
// файл СЛОТА. Один и тот же экран показывал бы разное на двух портах, и хуже
// того — запись легла бы третьим местом хранения ключа.
// 🔒 Имя переменной здесь — не `SLOT_ENV_PATH`: этот репозиторий уже называет
// её `FRACTERA_SLOT_ENV`, её пишет установщик, и её же читает `slotEnv()`.
// Заводить второе имя для одного пути значило бы развести их молча.
// Порядок: явный аргумент → имя стартера → имя зеркала → свой файл.
function envPath(path?: string): string {
  return (
    path ??
    process.env.SLOT_ENV_PATH ??
    process.env.FRACTERA_SLOT_ENV ??
