...
//
// 🔒 ЖИВЁТ В ОБЩИХ КОМПОНЕНТАХ, А НЕ В ПАПКЕ МАРШРУТА (181-7). Потребителей стало
// двое: страница `/terminal` и островок активации бота на вкладке «Подключение
// Telegram-бота». Третья копия разошлась бы с остальными молча — тот же закон, что
// у ленты сообщений и у каталога блоков.
// 🛑 ИСКЛЮЧЕНИЕ НАЗВАНО: у страницы входа `claude-subscription` СВОЯ копия, и это
// намеренно — она байт в байт совпадает со страницей входа службы памяти, а та
// живёт в другом репозитории и общих импортов с нами не имеет (закон 137).
//
