  ...
  const head =
    known === 0
      ? [
          ...
          `Память предлагает спросить: «${acquaint.ask}» — ${acquaint.why}.`,
          "Ответ без вопроса о человеке оставит память пустой навсегда.",
        ].join(String.fromCharCode(10))
      : [
          `🔔 ЗНАКОМСТВО ИДЁТ: известно ${known}, осталось спросить ${acquaint.left}.`,
          `Следующий вопрос — «${acquaint.ask}» (${acquaint.why}).`,
          "Спрашивай, только когда это к месту: один вопрос за обмен, не анкетой.",
        ].join(String.fromCharCode(10));
