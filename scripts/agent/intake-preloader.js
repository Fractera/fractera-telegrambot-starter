...
const TOOL = {
  // 🔒 ОПИСАНИЕ НАЧИНАЕТСЯ С ТОГО, КОГДА НЕ ЗВАТЬ (171-2), И ЭТО НЕ СТИЛЬ.
  // Описание читается агентом В МОМЕНТ РЕШЕНИЯ, инструкция — в начале сессии;
  // первое сильнее. ✗ измерено 2026-09-09: этот инструмент стоит 10 777 мс —
  // в 110 раз дороже чтения памяти, — потому что внутри два вызова внешней
  // модели, вектор и граф. На «привет» всё это не нужно ничему.
  description:
    "ТОЛЬКО для дороги ③ (работа) и для ЛЮБОГО присланного файла. " +
    "НЕ ЗВАТЬ на приветствие, благодарность, общий вопрос и вопрос о самом человеке: " +
    "стоит около 10 секунд и на этих дорогах не нужен — там хватает reply или memory_read. " +
    "Что делает: принимает сообщение или файл во все хранилища проекта и возвращает готовый текст. " +
...
async function runIntake(a) {
  ...
  return [String(r.forAgent || "(дверь не вернула текст)"), await acquaintanceHint()]
    .filter(Boolean)
    .join(String.fromCharCode(10, 10));
}

// ── ЗНАКОМСТВО ПРИЕЗЖАЕТ ТУДА, КУДА АГЕНТ ХОДИТ ВСЕГДА (169-1) ─────────────
//
// 🎯 РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-09-09: «на уровне инструкции нужно просто создать
// функцию, которая механически мгновенно ответит: это начало диалога —
// активировать навык первого знакомства».
//
// ✗ ЧЕМ ОПЛАЧЕНО, И ЭТО ИЗМЕРЕНИЕ, А НЕ ДОГАДКА. Живой прогон 2026-09-09: на три
// сообщения подряд («Привет», «Ты кто», рабочий вопрос) агент позвал `intake`
// ТРИ раза из трёх, а `memory_read` — НОЛЬ. Предложение знакомства живёт в
// ответе `memory_read`, то есть не приходило никогда. В базе остался ноль.
//
// 🔒 ПОЧЕМУ ЭТО НАДЁЖНЕЕ ИНСТРУКЦИИ, И РАЗНИЦА МЕХАНИЧЕСКАЯ. Инструкция читается
// один раз при старте и исполняется настолько, насколько модель помнит её в этот
// ход. Ответ инструмента приходит **в момент решения**, прямо перед следующим
// шагом. Правило, ставшее конструкцией, не зависит от памяти модели.
//
// 🔒 РЕШАЕТ ФАКТ ПУСТОЙ ПАМЯТИ, А НЕ ФОРМА ФРАЗЫ. Приветствие можно не узнать в
// лицо: человек начинает с «слушай, посчитай мне…». Считается очередь знакомства
// — сколько признаков с `askOrder` уже названо.
const ACQUAINT_MAX_HINTS = 3;
let acquaintKnown = null; // null — не спрашивали; true — знакомство состоялось
let acquaintHints = 0;

async function acquaintanceHint() {
  // 🔒 УЗНАЛИ ОДИН РАЗ — БОЛЬШЕ НЕ СПРАШИВАЕМ. Процесс живёт долго, и лишний
  // поход в память на каждом сообщении стоил бы 46 мс без всякой пользы.
  if (acquaintKnown === true) return "";
  // 🛑 НЕ ДАВИМ. Человек, промолчавший трижды, не хочет — и это его право;
  // четвёртое напоминание превращает знакомство в допрос.
  if (acquaintHints >= ACQUAINT_MAX_HINTS) return "";

  const r = await postOwn(MEMORY_URL, { args: { limit: 20 }, fn: "read" });
  const answer = (r && r.answer) || {};
  const acquaint = answer.acquaint || null;

  // Очередь знакомства кончилась — память перестала предлагать вопросы.
  if (!acquaint) {
    acquaintKnown = true;
    return "";
  }
  ...
  const known = items.filter((i) => i && i.about === "self").length;
  acquaintHints += 1;

  // ── ЯЗЫК ИДЁТ РАНЬШЕ ВСЕГО ОСТАЛЬНОГО (170-3) ───────────────────────────
  //
  ...
  //
  // 🔒 ЯЗЫК НАБЛЮДАЕТСЯ, А НЕ СПРАШИВАЕТСЯ, И РЕШАЕТ ЭТО АГЕНТ. Механически
  // различить кириллицу русского, украинского и болгарского нельзя; модель видит
  // текст и понимает язык. Наше дело — напомнить об этом в момент решения, а не
  // угадывать кодом.
  // 🛑 ЧЕЛОВЕК МОГ НАПИСАТЬ НЕ НА СВОЁМ ЯЗЫКЕ: интерфейс английский, а говорить
  // удобнее по-русски. Поэтому наблюдение записывается ПРЕДПОЛОЖЕНИЕМ, и в
  // ответе человеку предлагается поправить.
  // 🛑 ЗАПИСЬ ЯЗЫКА — ОДИН ВЫЗОВ ВМЕСТЕ С ОТВЕТОМ, А НЕ ОТДЕЛЬНЫЙ ЗАХОД.
  // Тот же закон о ходах модели: «сначала запиши, потом отвечай» стоило бы двух
  // проходов там, где хватает одного.
  const hasLanguage = items.some((i) => i && i.key === "person.language");
  const languageLine = hasLanguage
    ? ""
    : [
        "🌐 Язык не записан. Отвечай на языке человека и заодно запиши его —",
        "   memory_write key=person.language claim=guess basis=«написал на этом языке».",
        "   Это одним вызовом рядом с ответом, отдельного захода не нужно.",
      ].join(String.fromCharCode(10));

  // 🛑 ЗДЕСЬ ГОТОВОЕ К ПРОИЗНЕСЕНИЮ, А НЕ ОТПРАВКА ЗА НАВЫКОМ (170-6).
  //
  // ✗ ИЗМЕРЕНО ЖИВЬЁМ 2026-09-09, И ЭТО БЫЛ МОЙ ДЕФЕКТ ЦЕЛИКОМ. Первая редакция
  // писала «Открой навык first-contact» — агент послушно открывал, читал ещё и
  // `SOUL.md`, звал `intake` второй раз, и ответ на «привет» занял **1 минуту 2
  // секунды**. Память при этом отвечала за **100 мс**: съели время не запросы, а
  // ШЕСТЬ ХОДОВ МОДЕЛИ.
  //
  // 🔒 ЗАКОН 158-4, КОТОРЫЙ Я САМ ЖЕ И НАРУШИЛ: дорого стоит ход модели, а не
  // запрос к базе. И слово владельца оттуда же: «мы создавали всю эту
  // технологию, чтобы не использовать избыточные запуски навыков».
  // 🔒 «ПРИВЕТ» — РАБОТА ОБЩЕГО РОДА, И ОТВЕТ НА НЕЁ ОБЯЗАН БЫТЬ МГНОВЕННЫМ.
  // Ради этого и построена воронка сепарации: простое отвечается сразу, логика
  // включается там, где она нужна. Отправить агента за документом на «привет»
  // значит сломать воронку в самой её точке.
  // 🛑 `SOUL.md` АГЕНТ УЖЕ ПРОЧИТАЛ ПРИ СТАРТЕ СЕССИИ — просить его прочитать
  // файл снова значит купить лишний ход за то, что уже лежит в контексте.
  const head =
    known === 0
      ? [
          "🔔 ПЕРВОЕ ЗНАКОМСТВО. Ответь ОДНИМ сообщением, без чтения файлов и навыков:",
          "   1) коротко представься — кто ты и зачем (это уже есть в твоей SOUL, она в контексте);",
          `   2) задай ровно один вопрос: «${acquaint.ask}» (${acquaint.why}).`,
          "   Больше ничего не вызывай: это работа общего рода, ответ должен прийти сразу.",
        ].join(String.fromCharCode(10))
      : [
          `🔔 ЗНАКОМСТВО ИДЁТ: известно ${known}, осталось ${acquaint.left}.`,
          `Уместный следующий вопрос — «${acquaint.ask}» (${acquaint.why}).`,
          "Задай его, только если он к месту. Один за обмен, не анкетой.",
        ].join(String.fromCharCode(10));

  // 🔒 ФРАЗА ПРИХОДИТ ЭТАЛОНОМ СМЫСЛА, А ПРОИЗНОСИТСЯ НА ЯЗЫКЕ ЧЕЛОВЕКА.
  // Словарь держит `ru` и `en`; всё остальное переводит агент — короткая фраза
  // модели бесплатна, а словарь на сотню языков разошёлся бы с собой.
  const sayIn =
    acquaint.say_in && acquaint.ask_in && acquaint.say_in !== acquaint.ask_in
      ? `   Произнеси этот вопрос на языке «${acquaint.say_in}» — фраза выше дана как эталон смысла.`
      : "";

  return [head, sayIn, languageLine].filter(Boolean).join(String.fromCharCode(10));
}
...
const TOOL_SEPARATE = {
  // 🪦 «ПЕРВЫМ ДЕЛОМ НА КАЖДОЕ СООБЩЕНИЕ» ОТМЕНЕНО 2026-09-09 (171-2). Эта фраза
  // стояла здесь и делала воронку сепарации набором ИМЁН вместо набора дорог:
  // род назывался, а шли все одинаково. Измерено: на роде `general` дверь не
  // пишет НИЧЕГО (`separate/route.ts:103`) — вызов был бесплатен по данным и
  // платен ходом модели, то есть 8–12 секунд у человека с телефоном в руке.
  // 🔒 Решение владельца по этой развилке, дословно: «Ноль вызовов, счёта нет».
  description:
    "ТОЛЬКО для дороги ③ (работа): человек просит что-то сделать, построить, починить, напомнить, " +
    "или прислал файл. НЕ ЗВАТЬ на приветствие, благодарность, общий вопрос и вопрос о самом " +
    "человеке — там дорога ① (только reply) или ② (memory_read). " +
    "Что делает: называет род работы и заводит её. Возвращает строку о категории, " +
