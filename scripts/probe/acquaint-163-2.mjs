...
// 🔒 ВТОРОЙ НЕГАТИВНЫЙ КОНТРОЛЬ: ЗНАКОМСТВО НЕ ЛЕЗЕТ В ЧУЖОЙ ВОПРОС.
// Вопрос, на который ответ НАШЁЛСЯ, предложением не сопровождается.
const byKey = await read({ key: "person.city" })
say(byKey.found === true && !byKey.acquaint,
  `на вопрос с готовым ответом знакомство не предлагается`)
