...
// 🪦 ЗАМЕР ПЕРЕНЕСЁН С ЦЕЛОГО ЗАПРОСА НА САМИ УРОВНИ (162-6), И ЭТО ИСПРАВЛЕНИЕ
// ИЗМЕРЕНИЯ, А НЕ СМЯГЧЕНИЕ. ✗ оплачено прогоном пачки из тринадцати приборов:
// быстрый путь намерил 1305 мс — не потому, что подорожал, а потому, что движок
// разбирал фоновую очередь соседей. Внутри ОДНОГО ответа уровни сравнимы честно:
// у уровня 2 своё время, и оно обязано быть больше времени уровня 1.
const lv = (a, n) => (a?.levels ?? []).find(l => l.level === n)
const ms1 = lv(r.json.answer, 1)?.ms ?? 0
const ms2 = lv(r.json.answer, 2)?.ms ?? 0
say(ms2 > ms1, `глубина дороже быстрого пути ВНУТРИ одного ответа: уровень 2 ${ms2} мс против уровня 1 ${ms1} мс`)
console.log(`  · для сведения: весь запрос fast ${fastMs} мс, deep ${deepMs} мс`)
