...
// 🔒 ЗАМЕР БЕРЁТСЯ У САМОГО УРОВНЯ, А НЕ У ВСЕГО ЗАПРОСА, И ЭТО НЕ ПРИДИРКА.
// ✗ прогон 2026-09-08 покраснел на сравнении целых запросов: уровни 1 и 2 гуляют
// от прогретости связей, и «отказ дешевле» иногда не выполнялось при исправной
// способности. Уровень 3 при отказе стоит РОВНО ноль, при разрешении — сотни мс:
// это и есть доказательство, что дорогой путь не начинался.
say(l3?.ms === 0, `отказ стоил ${l3?.ms} мс внутри уровня`)
say(Number(l3ok?.ms) > 0,
  `с разрешением тот же уровень стоил ${l3ok?.ms} мс — значит при отказе он не начинался`)
console.log(`  · для сведения: весь запрос с отказом ${held.ms} мс, с разрешением ${allowed.ms} мс`)
