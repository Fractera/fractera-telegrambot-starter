...
`);
  // 🔒 МОСТ ЗДОРОВАЕТСЯ ПЕРВЫМ. Человек видит, что соединение установлено, ещё
  // до того, как оболочка напечатала приглашение; молчащая чёрная панель
  // неотличима от сломанной. Прибору эта строка отвечает на вопрос «жив ли
  // сокет в момент приёма» — тот, на котором сегодня застряла отладка.
  try {
    ws.send(String.fromCharCode(13,10) + "[мост принял соединение]" + String.fromCharCode(13,10));
  } catch (e) {
    process.stderr.write("[pty] приветствие не ушло: " + e.message + String.fromCharCode(10));
  }
  let proc = null;
  let started = false;
