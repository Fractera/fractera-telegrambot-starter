...
// 🔒 РАЗДЕЛОВ СТАЛО СЕМЬ, И ЧИСЛО ПРАВЛЕНО ТОЙ ЖЕ ПРАВКОЙ, ЧТО ТРОГАЕТ МАССИВ
// (2026-09-10, седьмым стал «Тест памяти»). Прежде здесь стояло «шесть».
export const TELEGRAM_SECTIONS = [
  ...
  "passport",
  // 🔒 «ТЕСТ ПАМЯТИ» — СЕДЬМОЙ РАЗДЕЛ С 2026-09-10 (шаг 176), И МЕСТО НАЗВАНО
  // ВЛАДЕЛЬЦЕМ ДОСЛОВНО: «на странице настройки ниже вкладки паспорт создай
  // вкладку Test памяти». Отсюда порядок: строго ПОСЛЕ `passport`.
  // 🔒 ЗАЧЕМ ОН НУЖЕН — ЧТОБЫ ИЗМЕРЯТЬ ПАМЯТЬ, А НЕ СУММУ «ПАМЯТЬ ПЛЮС АГЕНТ».
  // ✗ оплачено разбором 2026-09-10: на вопрос «что ты знаешь обо мне» от нажатия
  // «отправить» до ответа прошло 2 мин 13 с, и к памяти относились СЕКУНДЫ —
  ...
  // 🛑 СТЕНД НЕ ИМЕЕТ ОТНОШЕНИЯ НИ К ЖИВЫМ ЗАПИСЯМ, НИ К АГЕНТУ — НИ СЕЙЧАС, НИ
  // ПОТОМ. Это свойство раздела, а не его текущее состояние; слова владельца:
  // «не имеет никакого отношения никаким живым записям ни к какому реальному
  // агенту и потом не будет иметь отношения».
  "memory-test",
] as const;
