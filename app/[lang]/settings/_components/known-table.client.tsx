      ...
      <td className="px-3 py-2">
        {editing ? (
          ...
          <button
            ...
            type="button"
          >
            {/* 🛑 ПУСТОЕ ПОКАЗЫВАЕТ ПРИМЕР СЕРЫМ, А НЕ ПОДСТАВЛЯЕТ ЕГО ЗНАЧЕНИЕМ:
                выдуманное имя было бы ложью о том, что система знает. */}
            {row.value ?? (
              <span className="text-muted-foreground italic">{row.example ?? "—"}</span>
            )}
          </button>
        )}
      </td>
