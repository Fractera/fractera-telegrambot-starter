...
export async function TaskParseSection({
  ...
  // 🛑 БЕРЁМ ПЕРВЫЙ, А НЕ ПОСЛЕДНИЙ, И ЭТО НЕ ОПЕЧАТКА. `listAutomationRows`
  // сортирует `ORDER BY id DESC` — новейшее сверху. ✗ первая версия этой строки
  // брала `rows[rows.length - 1]` и показала бы САМУЮ СТАРУЮ автоматизацию под
  // словом «последняя»: измерено, в базе семь записей с наибольшим номером 9,
  // а показалась бы первая. **Позиция в массиве — не порядок во времени**, пока
  // не прочитан `ORDER BY`.
  const { rows: automationRows } = await listAutomationRows(50)
  const latest = automationRows[0] ?? null
