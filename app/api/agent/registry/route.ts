...
export async function POST(request: Request) {
  ...
  if (decl.fn === "remember") {
    ...
    const written = await writeFact({
      key,
      // 🔒 ЗНАЧЕНИЕ ПЕРЕДАЁТСЯ КАК ЕСТЬ, БЕЗ ПРИВЕДЕНИЯ К СТРОКЕ.
      // ✗ НАЙДЕНО ПРИБОРОМ 160: здесь стояло `String(args.value ?? "")`, и объект
      // превращался в строку ДО сторожа глубины. Сторож видел строку, пропускал её,
      // и `{name, car: {model}}` спокойно ложился в личную таблицу — то есть запрет
      // существовал, а данные второго порядка всё равно проходили.
      // 🛑 КЛАСС ШИРЕ СЛУЧАЯ: приведение типа ПЕРЕД проверкой обезоруживает проверку.
